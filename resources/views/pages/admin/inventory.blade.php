...
<header class="py-2.5 px-6">
    ...
    <div class="flex items-center flex-wrap gap-2">
        <form action="" method="GET" class="text-gray-800 relative grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 xl:max-w-5xl gap-x-2 gap-y-4">
            <select name="category" id="category" class="px-2 py-2 outline-indigo-500 text-lg rounded-md"
            ...
            <select name="location" id="location" class="px-2 py-2 outline-indigo-500 text-lg rounded-md"
                aria-label="Select location">
                <option value="">All Locations</option>
                @foreach ($locations as $location)
                <option value="{{ $location->name }}" @selected($location->name === request('location'))>{{ $location->name }}</option>
                @endforeach
            </select>
            @role("admin")
            <select name="added_by" id="added_by" class="px-2 py-2 outline-indigo-500 text-lg rounded-md"
                ...
                <option value="{{ auth()->user()->uuid }}" @selected(auth()->user()->uuid === request('added_by'))>Admin</option>
                <optgroup label="Managers">
                    @foreach ($users['manager'] as $manager)
                    <option value="{{ $manager->uuid }}" @selected($manager->uuid === request('added_by'))>{{ $manager->name }}</option>
                    @endforeach
                </optgroup>
                <optgroup label="Employees">
                    @foreach ($users['employee'] as $employee)
                    <option value="{{ $employee->uuid }}" @selected($employee->uuid === request('added_by'))>{{ $employee->name }}</option>
                    @endforeach
                </optgroup>
            </select>
            @endrole
