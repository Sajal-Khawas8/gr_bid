...
<article class="py-6 space-y-8 min-h-[calc(100vh-4rem-3.5rem)] flex flex-col justify-center">
    <h1 class="text-center text-4xl font-semibold"> Login to GRBid </h1>
    <form action="{{ route('login') }}" method="post" class="space-y-8 w-full max-w-md mx-auto">
        @csrf
        <div>
        ...
        <a href="{{ route('password.request') }}" class="text-indigo-600 font-medium inline-block">Forgot Password?</a>
